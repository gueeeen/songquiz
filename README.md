...
題庫每個語種 **450 首可出題 ＋ 500 個誘餌**（誘餌是只當錯誤選項、不會被出題的歌名），
五個語種合計 **2,250 首 ＋ 2,500 誘餌**。歌是從 Apple 的播放排行榜撈的，每個月重跑就會換新。
...
**不是直接覆蓋，是合併。** 預設 `--carry 0.30`：每語種 450 個名額裡，
315 個給這個月的新榜，135 個留給「上一版還在、這次沒撈到」的歌。
兩個理由：
...
| 參數 | 預設 | 說明 |
|---|---|---|
| `--per-artist` | 15 | 每位演出者取幾首進題庫 |
| `--decoys-per-artist` | 14 | 同一位再取幾首當誘餌 |
| `--tracks` | 450 | 每個語種的題庫上限 |
