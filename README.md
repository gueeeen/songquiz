...
每個語種 400 首，順序決定了難度怎麼切：

| 層 | 來源 | 每月會換 | 大約幾首 |
|---|---|---|---|
| ① | **榜上直接收前 50 首**（`--chart-songs`） | ✅ | 50 |
| ② | **經典歌單**（`tools經典歌單.js`） | ❌ | 80～205 |
| ③ | 榜單歌手用 Search API 撈到的歌 | ✅ | 補滿 400 |

**流動性來自第一層。** 「簡單」那一級如果全部是經典，它就是固定的，玩久了會背起來；
榜每個月會換一批，把它的前 50 名放在最前面，簡單那一級就跟著換。
榜單那 100 首本身就帶試聽網址，所以這一層不用多打一次 Search API。

**第二層是保底。** 榜衡量「這個月在紅」，猜歌需要「認得出來」——周杰倫、五月天、
蔡依林、伍佰、江蕙不一定每個月都在榜上，但它們最有把握被喊出來。
種子檔就是第一版的題庫（手挑歌手撈的 512 首），轉成新格式後進版控。

實作上沒有特例：三層各給一個量級的 `Fame`（榜上 −200000＋名次、經典 −100000＋順序、
搜尋來的 ≥0），難度分級照 `Fame` 排序自然就切出這個結果。
...
| `--chart-songs` | 50 | 每個語種直接從榜上收幾首（「簡單」的流動性來自這裡）|
| `--classics` | `tools經典歌單.js` | 經典種子檔；傳空字串就不種 |
...
**遊戲規則**（74 項）：打開 `web\tests.html`，最上面會顯示 `RESULT PASS 74/74`。

**版面**（50 項）：打開 `web\layout-tests.html`。它把 `index.html` 分別用 390px 和
...
**題庫產生器**（44 項，歌名清洗、題庫檔的格式契約、每月合併的往返、三層疊法與難度分級）：`dotnet test`。
